...
#
# Usage example:
#   python sts_eval_wmt20_enzh.py \
#       --api_base_url http://localhost:8000/v1 \
#       --api_key YOUR_KEY_HERE \
#       --model_name Meta-Llama-3.1-8B-Instruct \
#       --out_dir ./results_wmt20_enzh_run1 \
#       --limit 100 \
#       --icl_n 5
...
HF_DATASET_NAME = "samsoup/Samsoup-WMT2020-en-zh"
SCORE_MAX = 100.0
...
def build_user_message(test_s1, test_s2, icl_examples):
    ...
    Each example includes Sentence A, Sentence B, and a gold similarity in [0.00, 100.00].
    """
    parts = []
    ...
    parts.append(
        "Sentence A (English source): " + test_s1.strip() + "\n"
        "Sentence B (Chinese translation): " + test_s2.strip() + "\n\n"
...
def query_similarity_llm(
    ...
    payload = {
        ...
        "max_tokens": 10,  # we expect something like "87.23"
    ...
    # Parse first float; then clamp to [0, 100]
...
def sample_icl_examples(train_split, n):
    """
    Randomly sample n examples from the WMT20 EN-ZH train split and prepare them
    as few-shot / in-context learning demonstrations (0-100 raw scores).
    Each example stores sentence1, sentence2, and gold score in [0,100] with two decimals.
    If n <= 0, returns [].
    """
    if n is None or n <= 0:
        return []
...
def run_eval(api_base_url, api_key, model_name, out_dir, limit, icl_n):
    ...
    for i, row in tqdm(enumerate(test_split), total=len(test_split)):
        ...
        # Gold raw score in [0,100], rounded to 3 decimals
        gold_raw = round(float(row["score"]), 3)
        gold_norm = gold_raw / SCORE_MAX  # [0,1]
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate LLM translation similarity on WMT20 EN-ZH using 0–100 raw scores (and normalized 0–1)."
    )
    parser.add_argument(
        "--api_base_url",
        type=str,
