...
#
# Usage example:
#   python sts_eval_llm_mteb_stsbenchmark.py \
#       --api_key YOUR_KEY_HERE \
#       --model_name Meta-Llama-3.1-8B-Instruct \
#       --out_dir ./results_mteb_stsbenchmark \
#       --limit 200 \
#       --icl_n 5
#
# What this script does:
#   - Loads mteb/stsbenchmark-sts (train/validation/test).
#   - Prompts the model for 0..5 scores, up to 3 decimals.
#   - NOW computes metrics on BOTH:
#       * raw 0..5
#       * normalized 0..1 (divide by 5)
#   - Saves predictions + metrics + ICL examples.
...
SYSTEM_MSG = (
    ...
    "- Output a real-valued similarity score from 0 to 5, inclusive.\n"
    "- 5 = same meaning / paraphrases.\n"
    "- 2.5 ≈ partially similar with important differences.\n"
    "- 0 = completely unrelated meaning.\n\n"
...
def build_user_message(test_s1, test_s2, icl_examples):
    ...
    if icl_examples:
        ...
        for i, ex in enumerate(icl_examples, start=1):
            parts.append(
                ...
                f"Similarity (0..5): {ex['score_0_5_up_to_3']}\n"
            )
    ...
    parts.append(
        ...
        "Answer with ONLY the similarity score in the range 0..5 (inclusive), "
...
def query_similarity_llm(
    ...
    # Parse a float; allow integers or decimals; clamp to [0,5]
...
def sample_icl_examples(train_split, n):
    ...
    for idx in idxs:
        ...
        s1 = row["sentence1"]
        s2 = row["sentence2"]
        score_raw = float(row["score"])  # 0..5
        score_txt = f"{score_raw:.3f}"
        examples.append(
            {"sentence1": s1, "sentence2": s2, "score_0_5_up_to_3": score_txt}
        )
    return examples
...
def run_eval(api_key, model_name, out_dir, limit, icl_n, seed):
    ...
    for i, row in tqdm(enumerate(test_split), total=len(test_split)):
        ...
        s1 = row["sentence1"]
        s2 = row["sentence2"]
        gold_raw = float(row["score"])  # 0..5
        gold_norm = gold_raw / 5.0  # → [0,1]
        ...
        rows_for_dump.append(
            {
                ...
                "sentence1": s1,
                "sentence2": s2,
                "gold_similarity_0_5": float(f"{gold_raw:.3f}"),
                "llm_similarity_0_5": float(f"{pred_raw:.3f}"),
        ...
        if i % 100 == 0:
            print(f"[{i}] Gold 0..5: {gold_raw:.3f} | LLM 0..5: {pred_raw:.3f}")
    ...
    metrics = {
        ...
        "label_range": "0..5 (also reported normalized to 0..1 via x/5)",
    }

    print("=======================================")
    print(f"Pairs evaluated: {metrics['num_pairs_scored']}")
    print(f"RAW 0..5     → Pearson: {metrics['pearson_correlation_raw']:.4f}")
    print(f"RAW 0..5     → MSE:     {metrics['mse_raw']:.6f}")
    ...
    with open(preds_path, "w", encoding="utf-8") as f:
        f.write(
            "idx,sentence1,sentence2,gold_similarity_0_5,llm_similarity_0_5,"
            ...
            f.write(
                ",".join(
                    [
                        ...
                        esc(r["sentence1"]),
                        esc(r["sentence2"]),
                        f"{r['gold_similarity_0_5']:.3f}",
                        f"{r['llm_similarity_0_5']:.3f}",
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Evaluate LLM similarity on mteb/stsbenchmark-sts (0..5 labels)."
    )
