# Base v1 -- only free or init weights
...
class BasePrototypicalRegressor(pl.LightningModule):
    ...
    def compute_distances(self, x_embed: torch.Tensor) -> torch.Tensor:
        return _cdist(
            x_embed.unsqueeze(1), self._current_prototype_matrix().unsqueeze(0)
        ).squeeze(1)
    ...
    def compute_losses(self, predictions, targets, x_embed):
        ...
        if self.separation_weight > 0.0 and self.prototype_mode == "free":
            pairwise = torch.pdist(self._current_prototype_matrix(), p=2)
            sep = (
                (-pairwise.mean())
                if pairwise.numel() > 0
    ...
    def _gather_train_embeddings_and_examples(
        ...
        for batch in tqdm(dl_ordered):
            if mode == "raw":
                ...
                # copy strings to avoid aliasing
                ...
                for k in ("input_ids", "attention_mask", "token_type_ids"):
                    if k in batch:
                        Bcat.setdefault(k, [])
                        # clone() to detach from dataloader pin-memory buffers
        ...
        embeds_cpu = torch.cat(outs, dim=0)
        examples = (
            {"text": texts}
            if mode == "raw"
    ...
    def _select_indices(
        ...
        if method == "kmedoids":
            ...
            km.fit(X)
            # ✅ use exact medoid row indices (stable, unique)
            if (
                ...
                # Fallback (shouldn't happen with sklearn-extra): project centers
                centers = getattr(km, "cluster_centers_", None) or getattr(
                    km, "cluster_centers", None
                )
                if centers is not None:
                    idx = _nearest_point_indices(embeds_cpu, centers, k)
                else:
                    idx = _farthest_first(embeds_cpu, k)

            # Ensure uniqueness & exact length k
            uniq = list(dict.fromkeys(idx.tolist()))
            if len(uniq) < k:
                extra = _farthest_first(
                    embeds_cpu, min(k, embeds_cpu.size(0))
                ).tolist()
        ...
        if method == "kmeans++" or method is None:
            ...
            return _nearest_point_indices(embeds_cpu, centers, k)

        if method == "farthest":
            self.print(
                f"[proto:init] Farthest-point select (k={k})", flush=True
            )
            return _farthest_first(embeds_cpu, k)
        ...
        if method == "dbscan":
            ...
            if len([l for l in set(labels) if l != -1]) >= 1:
                idx = _cluster_means_to_indices(embeds_cpu, labels)
                # pad/truncate to k
                if idx.numel() < k:
                    extra = _farthest_first(
                        embeds_cpu, min(k, embeds_cpu.size(0))
                    ...
                    idx = idx[:k]
                return idx
            # fallback to kmeans++ if clusters invalid
    ...
    def _set_example_prototypes(
        ...
        if "text" in examples_full:
            ...
            # clone tensors so we don't alias the big cache
    ...
    def _ensure_prototypes_ready(self):
        ...
        if self.prototype_mode != "example":
            return
        # already present from training or restored from ckpt
    ...
    def _maybe_move_prototypes_to_device(self):
        ...
        # prototype_examples stay on CPU or as Python lists; we move to device on use.

    # ------------- trainer lifecycle hooks -------------
    def setup(self, stage: Optional[str] = None):
        """
        Called on trainer startup for any stage ('fit', 'validate', 'test', 'predict').
        We use it to guarantee exemplar selection before a pre-train smoke test or validation,
        and to migrate restored prototypes to the right device.
        """
    ...
    def on_fit_start(self):
        # safety (no-op if already selected in setup)
        self._ensure_prototypes_ready()
        self._maybe_move_prototypes_to_device()
        self.dump_trainable_status()
        self.print(
            f"[hparams] mode={self.prototype_mode}, selector={self.prototype_selector}",
            flush=True,
        )
    ...
    def on_load_checkpoint(self, checkpoint: Dict[str, Any]) -> None:
        ...
        if self.prototype_mode == "example":
            # keep on CPU here; move later in setup/on_*_start
